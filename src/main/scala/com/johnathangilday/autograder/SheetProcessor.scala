...
trait SheetProcessorComponentImpl extends SheetProcessorComponent {

  override val sheetProcessor: SheetProcessor = new SheetProcessorImpl
  ...
  class SheetProcessorImpl extends SheetProcessor with Logging {
    ...
    def processImage(bufImg: BufferedImage): Seq[Seq[Boolean]] = {
      ...
      val rows = findRows(binaryNoNoise, circles)
      findMarks(binaryNoNoise, rows)
    }
